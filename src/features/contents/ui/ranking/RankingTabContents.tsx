...
const RankingTabContents = ({ getImageSrc, onCategoryChange }: RankingTabContentsProps) => {
  ...
  useEffect(() => {
    ...
    if (ottParts.length > 0) {
      console.log('RankingTabContents: Syncing selectedOTTs with store category parts:', ottParts);
      setSelectedOTTs(ottParts);
    } else if (!currentCategory) {
      // 카테고리가 비어있으면 selectedOTTs도 초기화
      console.log('RankingTabContents: Resetting selectedOTTs due to empty category');
      setSelectedOTTs([]);
    }
  }, [currentCategory]);
  ...
  const handleOTTSelect = (ott: string) => {
    console.log('RankingTabContents: OTT selected:', ott, 'Previous:', selectedOTTs);
    ...
    if (selectedOTTs.includes(ott)) {
      ...
      newSelectedOTTs = selectedOTTs.filter(selectedOtt => selectedOtt !== ott);
    } else {
      // 선택되지 않은 OTT인 경우 추가
    ...
    try {
      ...
      // 카테고리 변경 콜백 호출
      const effectiveCategory = getEffectiveCategory(uniqueOtts, appliedFilters);
      onCategoryChange?.(effectiveCategory);
    } catch (error) {
  ...
  const renderContent = (selectedCategory: string) => {
    const effectiveCategory = getEffectiveCategory(selectedOTTs, appliedFilters);
    
    return (
      <>
        <div className="flex justify-between items-center mb-4">
          <OTTSelector activeOTTs={selectedOTTs} onSelectOTT={handleOTTSelect} />
